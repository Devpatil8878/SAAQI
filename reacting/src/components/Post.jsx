...
function Post({isDarkMode}) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
        // Perform post submission logic here, for example, using fetch or axios
        ...
        if (response.ok) {
            ...
            // Optionally, you can perform additional actions after successful post submission
        } else {
        ...
        <div className={`${isDarkMode ? "dark-mode-component-bg" : "light-mode-component-bg"} post-animation w-[70%] h-40 rounded-xl mx-10 flex flex-col`}>
            <form onSubmit={handleSubmit}>
                ...
                <Link to="/account">
                    ...
                    <textarea name="postText" value={postContent} onChange={(e) => setPostContent(e.target.value)}
 id="posttext" cols="80" rows="3" placeholder="What's in your mind?" className="peer h-[60%] min-h-[50%] w-[75%] resize-none outline outline-0 transition-all disabled:resize-none mt-6 bg-transparent rounded-xl px-6 py-2 no-scrollbar" ></textarea>
                ...
                <div className="options flex justify-between">
                    <div className="flex">
                        ...
                        <button className='w-8 h-8 global-theme-color flex justify-center items-center rounded-full ml-4 mb-11 min-w-8'>
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={`text-white w-4 h-4`}>
